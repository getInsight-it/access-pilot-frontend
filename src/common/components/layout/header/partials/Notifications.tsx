        ...
        <Popover open={isOpen} onOpenChange={handleOpenChange}>
          <PopoverTrigger asChild>
            <Button variant="outline" size="icon">
              <Bell className="h-[1.2rem] w-[1.2rem] rotate-0 scale-100 transition-all text-default" />
            </Button>
          </PopoverTrigger>
          <PopoverContent className="w-[calc(100vw-2rem)] md:w-[480px] p-0 max-h-[60vh] flex flex-col z-[100]">
            <header className="flex flex-row justify-between items-center p-4">
              <h4 className="text-lg font-semibold">Notificações</h4>
              <div className="flex flex-row gap-1 items-center cursor-pointer">
                {/* TODO trocar cor para accent */}
                <CheckCheck className="h-4 w-4 text-blue-500" />
                <span className="hidden sm:inline text-sm text-blue-500">Marcar todas como lido</span>
                <span className="sm:hidden text-sm text-blue-500">Marcar lidas</span>
              </div>
            </header>
            <Separator />
