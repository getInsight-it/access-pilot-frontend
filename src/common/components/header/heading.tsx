      ...
      {(items && items.length) && items.map((item, index) => {
        ...
        return (
          <div className="flex flex-col w-full gap-2" key={index}>
            <div className={`h-[4px] w-full ${color === "primary" ? "bg-primary-600" : "bg-gray-200"}`}></div>
            <span className={`text-sm font-semibold ${active ? "text-primary-700" : "text-gray-700"}`}>{item}</span>
          </div>
        );
      })}
    </div>
    ...
    <div className="flex flex-col gap-4 w-full">
      <div className="flex flex-row gap-4 w-full">
        {returnButton && (
          <Button
            variant="outline"
            className="border-primary-600 dark:border-primary-600 w-8 h-8 p-0"
