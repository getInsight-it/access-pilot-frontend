...
export default function CreateOrEditLevel() {
  ...
  const fetchSphereData = async (id: string) => {
    try {
      ...
      // Log completo dos dados recebidos da API
      console.log("Dados completos da esfera recebidos da API:", JSON.stringify(data, null, 2))

      console.log("Dados da esfera carregados:", data)
      ...
      try {
        ...
        if (onlyParentChanged) {
          ...
          if (isEditing && sphereId) {
            ...
            console.log("Atualizando esfera existente com ID:", sphereId)
          ...
          <div className="w-128 mt-4">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <Input className="mt-2" id="name" value={name} onChange={(e) => setName(e.target.value)} required />
              </div>
              <div>
              ...
              <div>
                <Label className="text-lg font-bold" htmlFor="sigla">
                  Sigla
                </Label>
                <Input
                  className="mt-2"
              ...
              <div>
                ...
                {/* Modificar o componente RadioGroup para ficar desabilitado quando estiver editando */}
                <RadioGroup
                  className="mt-2"
                  value={type}
                  onValueChange={(value: "BUSINESS" | "EXTERNAL") => {
                    setType(value)
                    // Resetar o estado de API key existente se mudar de tipo
                    if (value !== "EXTERNAL") {
                      setHasExistingApiKey(false)
                    }
                  }}
                ...
                {isEditing && (
                  <p className="text-sm text-blue-600 mt-1">O tipo da esfera não pode ser alterado após a criação.</p>
                )}
              </div>
              {type === "EXTERNAL" && (
                <>
                  <div>
                    <Label className="flex justify-between items-center text-lg font-bold" htmlFor="endpoint">
                      Endpoint
                    </Label>
                    <Input
                    ...
                    <div className="text-sm text-gray-500 flex items-center mt-3">
                      <Info className="w-5 h-5 mr-3" size="24" />
                      Para mais informações sobre a criação do seu endpoint,{" "}
                      <span className="bold text-black underline">clique aqui</span>{" "}
                    </div>
                  </div>
                  <div>
                    <Label className="text-lg font-bold" htmlFor="apiKey">
                      API Key
                    </Label>
                    <Input
                      ...
                      required={!isEditing} // Só é obrigatório para novas esferas
                    />
              ...
              <div>
                <Label className="text-lg font-bold" htmlFor="parentSphere">
                  Esfera pai
                </Label>
                <div className="mt-2">
                  ...
                  <Select
                    value={parentId ?? "0"}
                    onValueChange={(value) => {
                      console.log("Alterando parentId de", parentId, "para", value)
                      setParentId(value)
