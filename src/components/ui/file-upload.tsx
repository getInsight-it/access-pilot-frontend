...
export const FileUpload = ({
  ...
  const isDisabled = files.length >= 3;

  const handleFileChange = (newFiles: File[]) => {
    setFiles((prevFiles) => {
      const totalFiles = prevFiles.length + newFiles.length;
      if (totalFiles > 3) {
        return [...prevFiles, ...newFiles.slice(0, 3 - prevFiles.length)];
      }
  ...
  return (
    <div className="w-full h-full" {...getRootProps()}>
      <motion.div
        ...
        className={cn(
          "p-10 group/file block rounded-lg cursor-pointer w-full h-full relative overflow-hidden",
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <p className="relative z-20 font-sans font-normal text-neutral-400 dark:text-neutral-400 text-base mt-2">
            {isDisabled
              ...
              : "Arraste aqui os arquivos ou clique para carregar"}
          </p>
