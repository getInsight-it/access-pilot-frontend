...
import Steps from "../steps/Steps4";
...
export function StepForm4() {
  ...
  const [invalidSteps, setInvalidSteps] = useState<boolean[]>([false, false, false]);
  const [isConfirmModalOpen, setIsConfirmModalOpen] = useState(false);
  const steps = [0, 1, 2];
  const stepLabels = ["Qual sistema precisa de acesso?", "E qual o seu papel?", "Conte-nos o motivo"];
  ...
  const handleNewRequest = () => {
    ...
    setInvalidSteps([false, false, false]);
  };  
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (currentStep === steps.length - 1) {
      ...
      if (result) {
        ...
        const newInvalidSteps = [false, false, false];
        if (errors.clientId) {
          errorMessage += "\n- Sistema";
  ...
  const renderStep = (step: number) => {
    switch (step) {
      case 0:
        return (
          <FormField
            ...
            name="clientId"
            render={({ field }) => (
              <FormItem className="relative min-h-[290px]">
                {/* <FormLabel>Selecionar o sistema</FormLabel> */}
                <FormControl>
                  <div>
                    <div
                      onClick={(e) => {
                  ...
                  <div className="">
                    <div className="grid grid-cols-2 gap-4">
                      {roles.map((role) => (
                        ...
                        >
                          <CardShine>
                            <div className={`p-4 text-left ${selectedRole === role.name ? 'border border-primary text-primary' : ''}`}>
                              {selectedRole === role.name && <Check className="w-5 h-5 absolute top-2 right-2 flex-shrink-0" />}
                              <p>{role.name}</p>
                            </div>
                          </CardShine>
                        </div>
      ...
      case 2:
        return (
          <div className="space-y-2 min-h-[290px]">
            <FormField
              control={form.control}
              name="description"
              render={({ field }) => (
                      ...
                      {attachments.length > 0 && (
                        <div className="mt-3 space-y-0">
                          {attachments.map((file, index) => (
                            <div key={index} className="bg-gray-200 pl-4 pr-2 py-0 rounded">
